...
;
;-
FUNCTION CURVEFIT_MOD, x, y, weightsIn, a, sigma, FUNCTION_NAME = Function_Name, $
    FITA=fita, $
...
;            Evaluate function and estimate partial derivatives
          ...
          alpha = DOUBLE(TRANSPOSE(pder[*,iparam])) # $
            DOUBLE((Weights # (FLTARR(nparam)+1)*pder))
          ...
          REPEAT BEGIN
             ...
             IF TOTAL(FINITE(b)-1) NE 0 THEN STOP
             ...
             IF (finite(chisq) EQ 0) OR $
                  (lambdaCount GT 300 AND chisq GE chisq1) THEN BEGIN

                ; Reject changes made this iteration, use old values.
                IF FINITE(chisq) EQ 0 THEN BEGIN
                    PressEnter, "ChiSq not finite! Hit return to Continue"
                ENDIF
